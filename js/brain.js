...
function draw() {
  ...
  for (let synapse of brain.synapses) {
    ...
    ctx.bezierCurveTo(point1.x + perp.x * width, point1.y + perp.y * width, point2.x + perp.x * widthMiddle, point2.y + perp.y * widthMiddle, middle.x + perp.x * widthMiddle, middle.y + perp.y * widthMiddle);
    ctx.bezierCurveTo(point3.x + perp.x * widthMiddle, point3.y + perp.y * widthMiddle, point4.x + perp.x * width, point4.y + perp.y * width, synapse.to.x + perp.x * width, synapse.to.y + perp.y * width);
    ctx.lineTo(synapse.to.x - perp.x * width, synapse.to.y - perp.y * width);
    ctx.bezierCurveTo(point4.x - perp.x * width, point4.y - perp.y * width, point3.x - perp.x * widthMiddle, point3.y - perp.y * widthMiddle, middle.x - perp.x * widthMiddle, middle.y - perp.y * widthMiddle);
    ctx.bezierCurveTo(point2.x - perp.x * widthMiddle, point2.y - perp.y * widthMiddle, point1.x - perp.x * width, point1.y - perp.y * width, synapse.from.x - perp.x * width, synapse.from.y - perp.y * width);
    ctx.fill();
  }
  for (let electron of brain.electrons) {
    ...
    ctx.arcTo(upSide.x + Math.cos(direction) * electron.size, upSide.y + Math.sin(direction) * electron.size, fontSide.x, fontSide.y, electron.size);
    ctx.arcTo(downSide.x + Math.cos(direction) * electron.size, downSide.y + Math.sin(direction) * electron.size, downSide.x, downSide.y, electron.size);
    // ctx.lineTo(downSide.x, downSide.y);
    ctx.bezierCurveTo(downSide.x - Math.cos(direction) * electron.size, downSide.y - Math.sin(direction) * electron.size, backSide.x + Math.cos(direction) * electron.size, backSide.y + Math.sin(direction) * electron.size, backSide.x, backSide.y);
    ctx.bezierCurveTo(backSide.x + Math.cos(direction) * electron.size, backSide.y + Math.sin(direction) * electron.size, upSide.x - Math.cos(direction) * electron.size, upSide.y - Math.sin(direction) * electron.size, upSide.x, upSide.y);
    ctx.fill();
  }
  for (let neuron of brain.neurons) {
